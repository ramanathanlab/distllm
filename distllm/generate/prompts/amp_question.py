...
class AMPQuestionPromptTemplate:
    # TODO:  insert your template, see question_answer.py for example 
    # with format strings
    template = (
    ...
    def _format_input(self, text: str ) -> str:
        ...
        return self.template.format(protein_name=protein_name, function_description=function_description)
    ...
    def preprocess(
        ...
        # input text is a string with "{entry['Protein_Name']}::{entry['F"unction']}"
        # split the string into two parts"

        return list(map(self._format_input, text))

    def _postprocess_response(self, response: str) -> tuple[str, str, list[str]]:
        # Use a more robust regex to split the text, accommodating potential formatting issues.
        ...
        if len(parts) > 1:
            ...
            if options_start:
                ...
                for option in options_list:
                    option_label = option[:2]  # This is "A)", "B)", "C)", or "D)"
